...
#             return jsonify({'error': 'User not found'}), 404
#
#         # ✅ Step 2: Check if there are any khutbahs with summaries for this user
#         khutbahs_exist = (
#             session.query(Khutbah)
...
#             }), 200
#
#         # ✅ Step 3: Get one random khutbah with summary
#         khutbah = (
#             session.query(Khutbah)
#             .filter(Khutbah.UserId == user_id, Khutbah.Summary.isnot(None))
#             .order_by(func.newid())  # RANDOM selection
#             .first()
#         )
#
#         if not khutbah:
#             return jsonify({'error': 'Failed to fetch khutbah summary'}), 500
#
#         # ✅ Step 4: Update user's weekly progress and nurbits
#         user.WeeklyProgress += 1
#         goal_reached = False
#
#         if user.WeeklyProgress >= user.CurrentWeekGoal:
...
#         session.commit()
#
#         # ✅ Step 5: Respond with summary and user progress
#         return jsonify({
